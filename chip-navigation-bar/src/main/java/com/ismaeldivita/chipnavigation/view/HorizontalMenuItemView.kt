...
internal class HorizontalMenuItemView @JvmOverloads constructor(
    ...
    private val excontainer by lazy { findViewById<View>(R.id.cbn_item_external_container) }
    ...
    override fun reduce() {
        dismissText()
        excontainer.animate().scaleY(0.7f).scaleX(0.7f).translationY((excontainer.height/6).toFloat())
    }

    override fun restore() {
        showText()
        excontainer.animate().scaleY(1f).scaleX(1f).translationY((0).toFloat())
    }
