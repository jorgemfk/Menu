...
class ChipNavigationBar @JvmOverloads constructor(
    ...
    fun dismissText(id: Int) {
        getItemById(id)?.dismissText()
    }

    fun showText(id: Int) {
        getItemById(id)?.showText()
    }

    fun reduce(id: Int) {
        getItemById(id)?.reduce()
    }

    fun restore(id: Int) {
        getItemById(id)?.restore()
    }
    ...
    fun reduce() {
        getChildren()
            ...
            .forEach { it.reduce() }
        animate().translationY((-height/4).toFloat())
    }
